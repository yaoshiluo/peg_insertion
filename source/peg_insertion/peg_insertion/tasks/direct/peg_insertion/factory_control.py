...
def compute_dof_torque(
    ...
    # Apply axis mask 
    # task_ctrl_axes_mask = torch.tensor(cfg.ctrl.task_ctrl_axes, device=device).unsqueeze(0)
    # task_wrench_motion = task_wrench_motion * task_ctrl_axes_mask
    
    task_wrench += task_wrench_motion
    # task_wrench = task_wrench_motion + ctrl_target_fingertip_contact_wrench
    # print("ctrl_target_fingertip_contact_wrench:", ctrl_target_fingertip_contact_wrench.cpu().numpy())
    ...
    # 外力补偿项 τ_ext = J^T * F_ext
    # if contact_force_vec is not None:
    #     #构建 6D Wrench（前3维为力，后3维为0）
    #     wrench_ext = torch.zeros((num_envs, 6), device=device)
    #     wrench_ext[:, :3] = contact_force_vec  #无 torque
    #     tau_ext = jacobian_T @ wrench_ext.unsqueeze(-1)
    #     dof_torque[:, 0:7] += tau_ext.squeeze(-1)

    # Gravity compensation
    # gravity_comp = robot_view.get_gravity_compensation_forces()
    # dof_torque[:, 0:7] += gravity_comp[:, 0:7]
...
def do_force_control(
    ...
    # Initialize total task-space wrench with target wrench
    task_wrench_force = ctrl_target_fingertip_contact_wrench.clone()


    # Compute wrench error (force + torque)
    force_error, torque_error = get_wrench_error(
        num_envs,
        contact_force_vec=contact_force_vec,
        ctrl_target_fingertip_contact_wrench=ctrl_target_fingertip_contact_wrench,
        device=device,   
    )
    wrench_error = torch.cat((force_error, torque_error), dim=1)

    # Apply P control to wrench error
    task_wrench_control = _apply_wrench_feedback_gains(
        wrench_error=wrench_error,
        wrench_prop_gains=wrench_prop_gains,
    )

    task_wrench_force = task_wrench_force + task_wrench_control
    force_ctrl_axes_mask = torch.tensor(cfg.ctrl.force_ctrl_axes, device=device).unsqueeze(0)

    task_wrench_force = task_wrench_force * force_ctrl_axes_mask
    # print("task_wrench_force:", task_wrench_force.cpu().numpy())

    # # Mask control to selected axes
    # masked_task_wrench_control = force_ctrl_axes_mask * task_wrench_control

    # # Add control wrench to total wrench
    # task_wrench_force = task_wrench_force + masked_task_wrench_control

    # Map task-space wrench to joint torques
    jacobian_T = torch.transpose(jacobian, dim0=1, dim1=2)
    dof_torque[:, 0:7] = (jacobian_T @ task_wrench_force.unsqueeze(-1)).squeeze(-1)
    # dof_torque[:, 0:7] = (jacobian_T @ task_wrench_control.unsqueeze(-1)).squeeze(-1)


    return dof_torque

def get_wrench_error(
    num_envs,
    contact_force_vec,
    ctrl_target_fingertip_contact_wrench,
    device,
    ...
    """
    Compute force and torque error between measured and desired fingertip wrench.

    Args:
        num_envs (int): Number of parallel environments.
        left_finger_force (torch.Tensor): [num_envs, 3]
        right_finger_force (torch.Tensor): [num_envs, 3]
        ctrl_target_fingertip_contact_wrench (torch.Tensor): [num_envs, 6]
        device (torch.device): Torch device to allocate tensors.

    Returns:
        force_error (torch.Tensor): [num_envs, 3]
        torque_error (torch.Tensor): [num_envs, 3]
    """
    # Combine measured fingertip forces (no torque sensor assumed)
    fingertip_contact_wrench = torch.zeros((num_envs, 6), device=device)
    fingertip_contact_wrench[:, 0:3] = contact_force_vec

    # Compute wrench error (note: measured wrench is a reaction, so negated)
...
def _apply_wrench_feedback_gains(
    wrench_error,
    wrench_prop_gains,
):
    """
    Apply proportional (P) gains to wrench error.

    Args:
        wrench_error (torch.Tensor): [num_envs, 6], force + torque error.
        wrench_prop_gains (torch.Tensor): [num_envs, 6], proportional gains.

    Returns:
        torch.Tensor: [num_envs, 6], control wrench.
    """
    task_wrench = torch.zeros_like(wrench_error)

    # Apply gains to force error (first 3 elements)
    task_wrench[:, 0:3] = wrench_prop_gains[:, 0:3] * wrench_error[:, 0:3]

    # Apply gains to torque error (last 3 elements)
    task_wrench[:, 3:6] = wrench_prop_gains[:, 3:6] * wrench_error[:, 3:6]
